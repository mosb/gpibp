function y = mvnlike(X, Mu, Sigma)
%MVNPDF Multivariate normal negative log likelihood .
%   Y = MVNLIKE(X) returns the negative log likelihood of the multivariate normal
...
%   N-by-1 vector.
%
%   Y = MVNLIKE(X,MU) returns the negative loglikelihood of the multivariate normal
...
%   case the negative log likelihood is evaluated for each row of X with the corresponding
%   row of MU.  MU can also be a scalar value, which MVNLIKE replicates to
%   match the size of X.
%
%   Y = MVNLIKE(X,MU,SIGMA) returns the negative log likelihood of the multivariate normal
...
%   the negative log likelihood is evaluated for each row of X with the corresponding page
%   of SIGMA, i.e., MVNLIKE computes Y(I) using X(I,:) and SIGMA(:,:,I).
...
%
%   If X is a 1-by-D vector, MVNLIKE replicates it to match the leading
